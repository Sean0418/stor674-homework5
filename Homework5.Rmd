...
# Commands you used
docker login 
docker tag stor674-hw5:latest sean2418/stor674-hw5:v1
```
...
<!-- Write your answer here -->
Apptainer is an open source container platform designed for high performance computing. It is used for HPC systems instead of Docker because Apptainer does not require root privileges so it is more secure. Apptainer also have immutable containers for signatures and provides single-file format easy for sharing. 
...
<!-- Write your answer here -->
BYOE means to create custom environments independent of the host system, like the operating system and the packages along with their specific versions to HPC, which will allow their code to run exactly as they did on the local machine. This allows for easier access and maintenance of reproducibility. 
...
apptainer pull docker://sean2418/stor674-hw5:v1
```
...
apptainer run stor674-hw5_v1.sif
```
...
GPU Computations:
Time for 10,000,000 elements: 0.0022 seconds
Time for 100,000,000 elements: 0.0198 seconds
...
```{r my-image-chunk, echo=FALSE, fig.cap="A screenshot of my job in the Slurm queue.", out.width="80%", fig.align="center"}
knitr::include_graphics("media/hw5-3.4-c.png")
```

```{r my-image-chunk, echo=FALSE, fig.cap="A screenshot of my job in the Slurm queue.", out.width="80%", fig.align="center"}
knitr::include_graphics("media/hw5-3.4-c2.png")
```
...
<!-- Write your answer here -->
This process eliminates the problem that the code only worked on my machine. Any researcher can pull the Git repository, build the same Docker image, and get the same results easily. The advantages are that it guarantees the same dependencies are used every time. The container can be ran anywhere HPC and local laptop alike. This approach also allows isolation of the environments so it doesn't interfere with other local environment and other user's environment. The potential challenges are the learning curve to use all of this technologies. The images.containers can also take up with a bit of space. 
